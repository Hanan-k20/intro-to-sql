...
JOIN countries c ON c.code=l.countrycode
...
SELECT 
	...
	s.name
FROM cities s
JOIN countries c ON c.code=s.countrycode
WHERE s.name ILIKE '%Serra'AND c.region ='South America';
...
SELECT 
	...
	c.capital 
FROM cities s
LEFT JOIN countries c ON c.capital=s.id 
WHERE c.code='BRA' ;
